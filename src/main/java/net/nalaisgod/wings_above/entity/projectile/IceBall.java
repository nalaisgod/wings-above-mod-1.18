...
public class IceBall extends ThrownItemEntity{
    private int duration = 200;


    public IceBall(EntityType<? extends IceBall> entityType, World world) {
        super((EntityType<? extends ThrownItemEntity>)entityType, world);
    }

    public IceBall(World world, LivingEntity owner) {
    ...
    public void tick() {
        ...
        if (this.world.isClient && !this.onGround) {
            for (int i = 0; i < 360; i++) {
                if (i % 20 == 0) {
                    this.world.addParticle(ParticleTypes.SNOWFLAKE, this.getX() + 0.5d, this.getY() + 1, this.getZ() + 0.5d,
