...
public class ModOtherBasicWings extends ArmorItem {
    public ModOtherBasicWings(ArmorMaterial material, EquipmentSlot slot, Settings settings) {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if(!world.isClient()) {
            if(entity instanceof PlayerEntity) {
                ...
                if(player.getInventory().getArmorStack(2).isOf(ModItems.JETPACK)) {
                    tick(player);
                    isGliding(player);
                }
            }
